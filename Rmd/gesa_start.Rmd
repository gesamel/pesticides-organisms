...
join2 = dbGetQuery(con, " SELECT DISTINCT ON (dsam.site_id, dsam.date, dsam.taxon) dsam.site_id, dsam.date, taxon, iz_n, cover, var.psm_type, var.name
                   FROM dia.dia_samples AS dsam
                   INNER JOIN views.psm_samples_2005 AS psam
                   ON dsam.site_id = psam.site_id
...
# Since iz_n (species abundance) is often empty,  I instead choose covefor abundance data.
...
join2$taxon <- sub("^(\\S*\\s+\\S+).*", "\\1", join2$taxon)
# Only 800 differet species left after shortening of the name.
```

# Subset only data from 2005 from the table join2005.
```{r  subset-2005-data, echo=TRUE, message=FALSE}
# within R and not via SQL query
join2005 <- subset(join2, date < as.Date("2006-01-01"))
# join2006 <- subset(join2, (date < as.Date("2007-01-01"))&(date > as.Date("2005-12-31")))
...
cast05 <- dcast(dia05, site_id ~ taxon, fill = 0) #amount of presence given
cast05 <- dcast(dia05, site_id ~ taxon, fill = 0, fun.aggregate = mean) #mean of cover

cast05 <-cast05[, -1] # removal of site information.
...
cast05_pa <-decostand(cast05, "pa")  #, na.rm = TRUE ??
# calculate sum per species
spec_sum <- apply(cast05_pa,2,sum)
sort(spec_sum)
...
spec_fin <- cast05[, ! spec_sum <2]

sort(apply(spec_fin, 2, max))
# 2 species remain with cover of 0.0 -> Nitzschia sigmoidea + Pinnularia microstauron
sort(apply(spec_fin, 2, sd))

```

```{r  dca, echo=TRUE, message=FALSE}
require(vegan)
spec_dca<- decorana(spec_fin)
spec_dca
...
freq1/nrow(join2)*100 # 0.3 % of all Samples-site combinations represent a species that was sampled only once.
