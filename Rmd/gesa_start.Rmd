...
join2 = dbGetQuery(con, " SELECT DISTINCT ON (dsam.site_id, dsam.date, dsam.taxon) dsam.site_id, dsam.date, taxon, psam.date, var.psm_type, var.name
                   FROM dia.dia_samples AS dsam
                   INNER JOIN views.psm_samples_2005 AS psam
                   ON dsam.site_id = psam.site_id
...
head(taxon_count)
nrow(taxon_count) # 931
nrow(taxon_count[taxon_count$Freq<5,]) # 383
nrow(taxon_count[taxon_count$Freq<3,]) # 274
nrow(taxon_count[taxon_count$Freq<2,]) #172
