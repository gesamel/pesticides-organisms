...
With state variable: explaiedintertia of July CCA is 18.53 % and of the September CCA it s 16.34 %.
...
```
The permutation test of the overall analysis indicates a very significant model for the month Jula (p = 0.009) and a very significant model for the month September (p = 0.0018).

The permutation test for each axis indicates a significant first axis (p = 0.019) for July. For the CCA for September, also only the first axis was significant (p = 0.011).

For July it seems that the variable state makes the CCA more meaningful, for September, the state variable does not to explain more of the diatom community.
...
```
July: cast_ge_cca[[i]] ~ ortho_phosphat + oxygen + state + o_phosphat_p 
is the most parsimonious model.
September: cast_ge_cca[[i]] ~ ortho_phosphat + conductivity + TOC 
With state variable, oxygen is not as important anymore.

```{r parsimonious-cca}
# July
dia_cca_pars_07 <- cca(cast_ge_cca[["07"]] ~ ortho_phosphat + oxygen + state + o_phosphat_p, cast_envi[["07"]])
dia_cca_pars_07
...
```
The explained (i.e. constrained) inertia of the July CCA was 18.53 % and of the September CCA it was 16.34 %. The explained inertia of the parsimonious CCAs was 12.09 % for July and 8.27 % for Septmeber.This drop is compensated by a clearer model with two significant canonical axes instead of one for both months.

The parsimonious CCA for July and September were both highly significant (p = 0.001)
The permutation test for each axis indicated that only the first axis of the July CCA were significant (p = 0.011). The permutation test for the September CCA indicates that  the first axis was highly significant ( p = 0.001) and the second axis significant (p = 0.011). 
...
MaxTU and sumTU of the full model CCA have very high VIFs (> 15000). This applies only to September. WHY??
For the parsimonious model of July, the state variables stateSN and stateST have still high VIFs (~10-11.  For the pars model of September, all VIFs are low (~1).
