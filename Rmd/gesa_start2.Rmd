...
phch_unique_siteid <-phch_sam[!duplicated(phch_sam$site_id),]

phch_sam_state <- phch_unique_siteid %>%
  mutate(state = sub('_.*$','', phch_unique_siteid$site_id)) %>%
  select(site_id, state)
# mutate matches the pattern i.e. first occurence of _ to the end of the string and replace with ''

phch_sam_state$state <- as.factor(phch_sam_state$state)

length(unique(phch_sam$site_id))
length(unique(phch_sam_state$site_id))
ncol(cast_envi[[2]])

# saveRDS(phch_sam_state, file.path('cache', 'phch_sam_state.rds'))
# phch_sam_state <- readRDS(file.path('cache', 'phch_sam_state.rds'))

phch_sam1 <- bind_rows(phch_sam, phch_sam_state)

#cast_envi_1 <- bind_cols(cast_envi[[1]], phch_sam_state)
# Do binding to both lsit elements:

lapply()
for (i in 1:length(cast_envi)) {
  bind_cols(cast_envi[[i]], phch_sam_state)
}

# Column `value_fin` can't be converted from numeric to character/ to factor

# Can a CCA be done on character values ??
```
...
```
The permutation test of the overall analysis indicates a significant model for the month Jula (p = 0.012) and a very significant model for the month September (p = 0.004).

The permutation test for each axis indicates a significant first axis (p = 0.020) for July. For the CCA for September, also only the first axis was significant (p = 0.007).
...
```
For July, the most parsimonious model is cast_ge_cca[[i]] ~ ortho_phosphat + oxygen
For September, the most parsimonious model is cast_ge_cca[[i]] ~ ortho_phosphat + conductivity + TOC + oxygen
...
# September
dia_cca_pars_09 <- cca(cast_ge_cca[["09"]] ~ ortho_phosphat + + conductivity + TOC + oxygen, cast_envi[["09"]])
dia_cca_pars_09
...
The permutation test for each axis indicated that both axes of the July CCA were significant (p = 0.001 and p = 0.002). The permutation test for the September CCA indicates that only the first two axes were significant (p = 0.001 and p = 0.012). 

The explained (i.e. constrained) inertia of the July CCA was 14.75 % and of the September CCA it was 13.49 %. The explained inertia of the parsimonious CCAs was 6.84 % for July and 8.03 % for Septmeber.This drop is compensated by a clearer model with two significant canonical axes instead of one for both months.

```{r vif}
lapply(cca_list, function(x) vif.cca(x))
lapply(cca_list_pars, function(x) vif.cca(x))
```
The parsimonious CCAs have very low variance inflation factors and thus, parsimony has paid off.
