...
##  species which occurred at 20 % of the sites
...
n_ge_abu <-length(unique(dia_sam_20ge$genus)) #32
n_ge_abu
n_ge_abu/n_ge*100 # 41.5 %

# Plot
dia_sam_N_20ge = dia_sam_20ge %>%
       group_by(genus) %>%
       summarise(N=n())
plot(density(dia_sam_N_20ge$N), main="Genus density")

# Rare species

dia_sam_20sp = dia_sam %>%
  group_by(species) %>%
...
n_sp_abu <- length(unique(dia_sam_20sp$species))
n_sp_abu #71
n_sp_abu/sp_n*100 #8.2 %

# Plot
dia_sam_N_20sp = dia_sam_20sp %>%
       group_by(species) %>%
       summarise(N=n())
plot(density(dia_sam_N_20sp$N), main="Species density")

###########################################################################################

## Rare = species that occur at less than 5 % of the samples
...
# precentage of genera remaining after removal of rare genera
n_5ge_abu <-length(unique(dia_sam_5ge$genus)) #44
n_5ge_abu
n_5ge_abu/n_ge*100 # 57.1 %

# Plot
dia_sam_N_5ge = dia_sam_5ge %>%
       group_by(genus) %>%
       summarise(N=n())
plot(density(dia_sam_N_5ge$N), main="Genus density")
...
# precentage of genera remaining after removal of rare genera
n_5sp_abu <-length(unique(dia_sam_5sp$species)) # 153
n_5sp_abu
n_5sp_abu/n_sp*100 # 17.7 %

# Plot
dia_sam_N_5sp = dia_sam_5sp %>%
       group_by(species) %>%
       summarise(N=n())
plot(density(dia_sam_N_5sp$N), main="Species density")
...
dia_aggr_month_20sp <- dia_sam_20sp %>% 
...
dia_aggr_split_20ge <- split(dia_aggr_20ge, dia_aggr_20ge$month)

cast_20ge <- lapply(dia_aggr_split_20ge, function(x) dcast(x, site_id ~ genus, value.var="perc95", fill = 0))

# Take the first column and store it as a rowname, afterwards, delete the first column.
cast_20ge <- lapply(cast_20ge,
                     function(x) { rownames(x) <- x[,1];
                                   x[,1] <- NULL;
                                   x })
...
lapply(cast_months_20ge, function(x) {decorana(x)})
lapply(cast_months_20ge, function(x) {plot(decorana(x), display="both")})
```
...
# conduct CA
ca_20ge <-lapply(cast_m20ge, function(x) {cca(x)})

# Compute summary of CA results to inspect proportion of variance explained by each axis.
lapply(ca_20ge, function(x) {summary(x)})
```
...
I.e.: 
July: 1.+2. CA: 22.1 % of cumulative explaned variance
August: 1.+2. CA: 21.5 % of cumulative explaned variance
September:  1.+2. CA: 26.2 % of cumulative explaned variance

```{r  plot-ca-month-ge, echo=TRUE, message=FALSE}
# Compute biplots without names
lapply(cast_20ge, function(x) {plot(cca(x), scaling=2)})

# Biplots with species names
dianame_long_20ge <- lapply(cast_20ge, function(x) colnames(x))

# dianame_m_ge <- lapply(cast_months_ge, function(x) make.cepnames(names(x)))
stems_20ge <- lapply(cast_20ge, colSums)

for (i in 1:length(dianame_long_20ge)) {
  plot(ca_20ge[[i]], dis="sp", type="n") #red crosses = species, dots = sites
  orditorp(ca_20ge[[i]], dis="sp", lab=dianame_long_20ge[[i]], priority=stems_20ge[[i]],
           pcol="red", pch="+")
}
```

#### Species - > 5% of samples

```{r cast-month, echo=TRUE, message=FALSE}
dia_aggr_month_5sp <- dia_sam_5sp %>% 
  group_by(month, species, site_id) %>%
  summarise(mean = mean(cover),
            perc95 = quantile(cover, 0.95, na.rm = TRUE),
            total = n()) %>%
...
#### Genus
```{r cast-month-ge-5, echo=TRUE, message=FALSE}
dia_aggr_5ge <- dia_sam_5ge %>% 
  group_by(month, genus, site_id) %>%
  summarise(mean = mean(cover),
            perc95 = quantile(cover, 0.95, na.rm = TRUE),
            total = n()) %>%
...
# Store single months as seperate elements of a list
dia_aggr_split_5ge <- split(dia_aggr_5ge, dia_aggr_5ge$month)

cast_5ge <- lapply(dia_aggr_split_5ge, function(x) dcast(x, site_id ~ genus, value.var="perc95", fill = 0))

# Take the first column and store it as a rowname, afterwards, delete the first column.
cast_5ge <- lapply(cast_5ge,
                     function(x) { rownames(x) <- x[,1];
                                   x[,1] <- NULL;
                                   x })
...
##### Genus
```{r dca-month-ge-5, echo=TRUE, message=FALSE}
lapply(cast_5ge, function(x) {decorana(x)})
lapply(cast_5ge, function(x) {plot(decorana(x), display="both")})
```
...
```{r  ca-month-ge-5, echo=TRUE, message=FALSE}
# conduct CA
ca_5ge <-lapply(cast_5ge, function(x) {cca(x)})

# Compute summary of CA results to inspect proportion of variance explained by each axis.
lapply(ca_5ge, function(x) {summary(x)})
```
...
```{r  plot-ca-month-ge, echo=TRUE, message=FALSE}
# Compute biplots without names
lapply(cast_5ge, function(x) {plot(cca(x), scaling=2)})

# Biplots with species names
dianame_long_5ge <- lapply(cast_5ge, function(x) colnames(x))

# dianame_m_ge <- lapply(cast_months_ge, function(x) make.cepnames(names(x)))
stems_5ge <- lapply(cast_5ge, colSums)

for (i in 1:length(dianame_long_5ge)) {
  plot(ca_5ge[[i]], dis="sp", type="n") #red crosses = species, dots = sites
  orditorp(ca_5ge[[i]], dis="sp", lab=dianame_long_5ge[[i]], priority=stems_5ge[[i]],
           pcol="red", pch="+")
}
