...
nrow(dia_sam[!is.na(dia_sam$cover), ])
nrow(dia_sam[!is.na(dia_sam$cover), ])/nrow(dia_sam)*100 # % of data that have cover as endpoint
...
gg3 <- ggplot(dia_sam, aes(x=year)) +
  geom_bar() +
  labs(x = "year", y = "number of diatom samples")
...
gg3 # + theme_grey(base_size = 18) 
ggsave(gg3, width = 10, height = 10, filename = "figures/n_year.png")
ggsave(gg3, width = 10, height = 10, filename = "Rmd/objects/n_year.png")

# number of samples per month
gg5 <- ggplot(dia_sam, aes(x=month)) +
  geom_bar() +
  labs(x = "month", y = "number of diatom samples")
gg5
ggsave(gg5, width = 4, height = 4, filename = "figures/n_month.png")
ggsave(gg5, width = 4, height = 4, filename = "Rmd/objects/n_month.png")
...
gg6 <- ggplot(sample_count_ym, aes(y = N, x = year)) +
  ...
  facet_wrap( ~ month) +
  labs(x = "year", y = "number of diatom samples") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title = element_text(size=15), strip.text.x = element_text(colour = "white", face = "bold", size = 14))
gg6
ggsave(gg6, width = 14, height = 10, filename = "figures/n_my.png")
ggsave(gg6, width = 13, height = 10, filename = "Rmd/objects/n_my.png")
...
gg7 <- grid.arrange(gg07, gg09, ncol=2)
...
n_ge_abu <-length(unique(dia_sam_ge$genus)) #45
n_ge_abu
n_ge_abu/n_ge*100 # 59.21 %
...
# precentage of genera remaining after removal of rare genera
n_sp_abu <-length(unique(dia_sam_sp$species)) # 152
n_sp_abu
n_sp_abu/n_sp*100 # 19.87 %
...
# precentage of genera remaining after removal of rare genera
n_ge_abu_cca <-length(unique(dia_sam_ge_cca$genus)) #41
n_ge_abu_cca
n_ge_abu_cca/n_ge_cca*100 # 66.12 %
...
Most abundant genera in CA and CCA dataset
```{r abundant-genera}
# CA dataset
dia_ge_list <- dia_sam_ge %>%
  select(genus) %>%
  group_by(genus) %>%
  summarise(total = n()) %>%
  arrange(desc(total))
...
# CCA dataset
dia_ge_list_cca <- dia_sam_ge_cca %>%
  select(genus) %>%
  group_by(genus) %>%
  summarise(total = n()) %>%
  arrange(desc(total))
...
sum(dia_sam_ge_cca$cover==0)

# Species occurences: Number of sampling dates where each species is present
...
#saveRDS(cast_ge, file.path('cache', 'dia_ge_ca_final.rds'))
cast_ge <- readRDS(file.path('cache', 'dia_ge_ca_final.rds'))
...
Most abundant genera and sites in July and September
```{r abundant-genera-per-month}
# CA dataset
gen_sum <- as.data.frame(sapply(cast_ge, function(x) colSums(x != 0)))

# CCA dataset
gen_sum_cca <- as.data.frame(sapply(cast_ge_cca, function(x) colSums(x != 0)))

```

```{r abundance-sites-per-month}
as.data.frame(rowSums(cast_ge[[1]] != 0))
as.data.frame(rowSums(cast_ge[[2]] != 0))

as.data.frame(rowSums(cast_ge_cca[[1]] != 0))
as.data.frame(rowSums(cast_ge_cca[[2]] != 0))
```
...
For genus data for the cca, the length of the first axis is > 3 S.D. in both months and therefore, a unimodal gradient can be assumed.
...
windows()
evplot(ev07)
savePlot(filename="Rmd/objects/ev_07", type="png")

ev09 <- ca_ge[[2]]$CA$eig
evplot(ev09)
savePlot(filename="Rmd/objects/ev_09", type="png")

# ca_ge[[1]]$CA$v   # how to extract species scores? these values are not correct..
...
# Plot July
windows()
plot(ca_ge[[1]], dis="sp", type="n",  xlim = c(-7.5,1), ylim = c(-4,2))
sel_07 <- orditorp(ca_ge[[1]], dis="sp", lab=dianame_long_ge[[1]], priority=stems_ge[[1]], pcol = "red", pch="+")
sel_07[1:45] # which genera are included in biplot

savePlot(filename="figures/plot_ca_07", type="png") 
savePlot(filename="Rmd/objects/plot_ca_07", type="png") 

# Plot September
plot(ca_ge[[2]], dis="sp", type="n", xlim = c(-6.5,1), ylim = c(-2,2))
sel_09 <- orditorp(ca_ge[[2]], dis="sp", lab=dianame_long_ge[[2]], priority=stems_ge[[2]], pcol = "red", pch="+")
sel_09[1:45] # which genera are included in biplot

savePlot(filename="figures/plot_ca_09", type="png")
savePlot(filename="Rmd/objects/plot_ca_09", type="png")

# Identify site and species names manually
#theme_set(theme_grey(base_size = 5))
#plot_ca_09 <-plot(ca_ge[[2]], main = "CA September", scaling = 2)
#identify(plot_ca_09, what="species")  # manually assign which species should be shown
#identify(plot_ca_09, what="sites")    # sites
```
...
Only include measurements taken in the same and the two previous years (total of3 years) of the diatom sample date.
```{r 3-years}
# Join on site_id with dia_sam_cca
# filter(date >= today() — years(1))
# filter(date >= today() - years(1))
library(lubridate)
filter(date >=  - years(2))
phch_sam$date - years(2)
```
...
phch_sam <- readRDS(file.path('cache', 'phch_sam_final.rds')) #11425 observations
...
length(cast_envi[[2]][[1]]) # 127 rows in total ??
...
# July
100/122*100 # 82 % agriculture data missing
90/122*100  # 74 % of o-phosphate-p data missing

# September
89/127*100 # 70 % agriculture data missing
58/127*100 # 45 % of o-phosphate-p data missing
...
# Plot
windows()
plot_cca_07 <- plot(dia_cca_07)
savePlot(filename="figures/plot_cca_07", type="png")  # window has to be open in order to save
# Plot only with species and envi variables, without site names
plot(dia_cca_07, scaling=2, display=c("species", "cn"))
savePlot(filename="rmd/objects/plot_cca_07_spenvi", type="png")

## September
# sumTU already removed
names(cast_envi[["09"]])
dia_cca_09 <- cca(cast_ge_cca[["09"]] ~ conductivity + maxTU + o_phosphat_p + + oxygen + pH  + temperature + TOC + tot_N + state, cast_envi[["09"]], na.action=na.omit)
dia_cca_09

# Plot
plot_cca_09 <- plot(dia_cca_09)
savePlot(filename="figures/plot_cca_09", type="png")

plot(dia_cca_09, scaling=2, display=c("species", "cn"))
savePlot(filename="rmd/objects/plot_cca_09_spenvi", type="png")
# dev.off()


cca_list <- list(dia_cca_07, dia_cca_09)
lapply(cca_list, function(x) summary(x))
...
```
The permutation test of the overall analysis indicates a non-significant model for the month July (p = 0.074) and a very significant model for the month September (p = 0.001).
...
```{r forward-selection}
for (i in 1:length(cast_ge_cca)) {
  ordistep(cca(cast_ge_cca[[i]]~1, data = cast_envi[[i]]), scope = formula(cca_list[[i]]), direction = "forward", pstep = 999)
}
```

July: cast_ge_cca[[i]] ~ pH + o_phosphat_p + conductivity  (+ tot_N) <-don't include tot_N
...
Save biplot scores for constraining variables in a table
```{r save-biplot-scores-as-dataframe}
# July
CCA1_07 = c(-0.6311, 0.6279, -0.4754)
CCA2_07 = c(-0.7756, -0.3761, 0.3085)
CCA3_07 = c(-0.0137, -0.6814, -0.8239)

biplot_scores_07 = data.frame(CCA1_07, CCA2_07, CCA3_07)
colnames(biplot_scores_07) <- c("CCA1", "CCA2", "CCA3")
rownames(biplot_scores_07) <- c("pH", "o_phosphate_p", "conductivity")

saveRDS(biplot_scores_07, file.path('Rmd/objects', 'biplot_scores_07.rds'))

# September
CCA1_09 = c(-0.93171, -0.07104, -0.66557, -0.69415)
CCA2_09 = c(0.02273, 0.89960, 0.06374, -0.12679)
CCA3_09 = c(-0.1435, -0.1239, 0.7380, 0.1753)
CCA4_09 = c(0.33289, -0.41270, 0.09098, -0.68654)

biplot_scores_09 = data.frame(CCA1_09, CCA2_09, CCA3_09, CCA4_09)
colnames(biplot_scores_09) <- c("CCA1", "CCA2", "CCA3", "CCA4")
rownames(biplot_scores_09) <- c("pH", "tot_N", "temperature", "conductivity")

saveRDS(biplot_scores_09, file.path('Rmd/objects', 'biplot_scores_09.rds'))
...
# July
plot(dia_cca_pars_07, scaling=2, display=c("species", "cn"))
savePlot(filename="rmd/objects/plot_cca_pars_07_spenvi", type="png")

plot(dia_cca_pars_07)
...
savePlot(filename="rmd/objects/plot_cca_pars_07", type="png")

# September
plot(dia_cca_pars_09, scaling=2, display=c("species", "cn"))
savePlot(filename="rmd/objects/plot_cca_pars_09_spenvi", type="png")

plot_cca_pars_09 <-plot(dia_cca_pars_09)
...
savePlot(filename="rmd/objects/plot_cca_pars_09", type="png")
...
```{r range-envi}
# in a nested loop
range_envi <- lapply(cast_envi, function(a) sapply(a[sapply(a,is.numeric)],range))
range_envi[[1]]
